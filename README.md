...
- ✅ 멀티 테넌시 — 사용자별 컬렉션 격리
- ✅ Rate limit + CORS 화이트리스트 + helmet
- ✅ AI 사용량 통제 (24h 캐시 + 일일 한도)
- ✅ S3 presigned URL — 백엔드 메모리 / 트래픽 부담 0
- ✅ Zod 기반 input validation
- ✅ PostgreSQL 이전

다음 단계 후보:
- **Google Maps 키 백엔드 프록시** — VITE_* 로 클라이언트에 노출된 키 회수
- **이미지 최적화** — 업로드 시 sharp 리사이즈 / WebP / EXIF strip (위치 정보 유출 차단)
